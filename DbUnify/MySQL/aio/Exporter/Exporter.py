...
class Exporter:
    ...
    async def export_chart_table(self, table_name: str, x_column: int, y_column: int, x_label: str = 'X Label', y_label: str = 'Y Label', title: str = 'Chart Title', save_path: str = 'chart.png', chart_type: str = 'bar') -> None:
        ...
        try:
            ...
            raise ChartCreationException(str(e))
    ...
    async def export_data_csv(self, table_name: str, csv_file_path: str, csv_file_name: str = 'output') -> None:
        ...
        Raises:
            CSVExportException: If there is an error during data export.
        """
        try:
            rows: List[tuple] = await self.manager.select(table_name)
            
            column_info = await self.manager.get_table_columns(table_name)
            header = [col[0] for col in column_info]
            ...
            async with aiofiles.open(f'{csv_file_path}/{csv_file_name}.csv', mode='w', newline='') as csv_file:
                await csv_file.write(output.getvalue())
        
        except Exception as e:
            raise CSVExportException(f"Error exporting data: {str(e)}")